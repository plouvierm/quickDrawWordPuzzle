...
module.exports = function (gulp, $, config) {
  ...
  // compile i18n files and copy into build directory
  ...
  // copy and static files build directory
  gulp.task('static', ['clean'], function () {
    return gulp.src(config.appStaticFiles, {dot: true})
      .pipe(gulp.dest(config.buildTempStatic));
  });
  ...
  gulp.task('deleteTemplates', ['copyTemplates'], function (cb) {
    var copyStaticFilesAndFolders = function () {
      gulp.src([config.buildTempStatic + '/**'], {dot: true})
        .pipe(gulp.dest(config.buildDir))
        .on('end', function () {
          $.del([config.buildTempStatic], {mark: true}, function () {
              cb();
            });
        });
    }
    ...
    if (!isProd) {
      ...
      var filesAndFoldersToRemove = [
        config.buildDir + '*',
        '!' + config.buildCss,
        '!' + config.buildTempStatic,
        '!' + config.buildFonts,
        '!' + config.buildImages,
        '!' + config.buildJs,
        '!' + config.extDir,
        '!' + config.buildDir + 'index.html',
        '!' + config.buildDir + '/angular'
      ];
      
      gulp.src([config.buildDir + '**/*.html'])
        .pipe(gulp.dest('tmp/' + config.buildDir))
        .on('end', function () {
          $.del(filesAndFoldersToRemove, {mark: true}, function () {
              copyStaticFilesAndFolders();
            });
        });
      }
  ...
  gulp.task('build', ['deleteTemplates', 'bowerAssets', 'images', 'fonts', 'static', 'i18n']);
};
